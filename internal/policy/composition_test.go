...
func TestComposeBaselinePlacementSetPreservesNotApplicableDecisions(t *testing.T) {
	...
	for _,step:=range out.Steps() {
		if step.Interpretation=="hard_allow" {allowed++}
		if step.Interpretation=="not_applicable" {notApplicable++}
	}
...
func TestComposeBaselineMissingInputBehaviors(t *testing.T) {
	...
	for _,tc:=range cases {
		t.Run(tc.name,func(t *testing.T){
			...
			if result.Outcome()!=tc.outcome {t.Fatalf("outcome=%q, want %q",result.Outcome(),tc.outcome)}
			if result.Steps()[0].Interpretation!=tc.code {t.Fatalf("first step=%#v",result.Steps()[0])}
		})
	}
}
